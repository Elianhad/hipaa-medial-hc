...
export function OrganizationPortalGuard({ children }: OrganizationPortalGuardProps) {
    ...
    const hasAccess = canAccessOrganizationPortal(roles);
    ...
    if (!hasAccess) {
        return (
            <main className="min-h-[70vh] bg-slate-950 text-white px-6 py-16">
                <div className="mx-auto max-w-4xl rounded-2xl border border-white/10 bg-white/5 p-8">
                    <h1 className="text-3xl font-semibold">Acceso Denegado</h1>
                    <p className="mt-3 text-slate-300">Tu rol actual no habilita acceso al portal de organizaciones.</p>
                    <div className="mt-6 flex flex-wrap gap-3">
